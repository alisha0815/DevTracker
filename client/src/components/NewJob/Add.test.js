...
describe('form validation', () => {
  ...
  test('after providing all inputs, the Add button is enabled', () => {
    ...
    expect(screen.getByRole('option', { name: 'frontend' }).selected).toBe(
      true
    );
    ...
    fireEvent.change(screen.getByTestId('date_applied'), {
      target: { value: '2020-05-12' },
    });
    expect(screen.getByTestId('date_applied')).toHaveValue('2020-05-12');

    fireEvent.change(screen.getByTestId('date_interview'), {
      target: { value: '2022-04-01T16:20' },
    });
    expect(screen.getByTestId('date_interview')).toHaveValue(
      '2022-04-01T16:20'
    );
  ...
  test('no error messages are present on page load', () => {
    ...
    expect(
      screen.queryByTestId('date_interview-error')
